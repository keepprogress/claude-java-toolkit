...
#        bash check-suppressions.sh --generate-allowlist <module> [project_root]
# 結束碼: 0=成功輸出結果（pass/fail/skip 透過 JSON status 傳遞）, 1=腳本 crash
#
# module:  模組名或 "." (單模組)
# filelist: 增量模式檔案清單。不提供 = 全量掃描。
# --generate-allowlist: 掃描現有 suppressions 並輸出 allowlist 到 stdout
#
...
if [[ $# -lt 1 ]]; then
    ...
    echo "       check-suppressions.sh --generate-allowlist <module> [project_root]" >&2
...
# --generate-allowlist mode: scan existing suppressions and output allowlist
GENERATE_ALLOWLIST=false
if [[ "$1" == "--generate-allowlist" ]]; then
...
# --generate-allowlist: scan all suppressions and output allowlist format
if [[ "$GENERATE_ALLOWLIST" == true ]]; then
    log "Generating suppression allowlist for $SRC_DIR..."
    echo "# Suppression allowlist — auto-generated by check-suppressions.sh"
    echo "# Format: relative/path/to/File.java:count"
    echo "# Edit counts or remove lines to tighten the gate."
    echo ""
    while IFS= read -r file; do
        count=$(grep -c 'CPD-OFF\|NOPMD' "$file" 2>/dev/null || echo "0")
        rel="${file#$MODULE_DIR/}"
        echo "$rel:$count"
    done < <(grep -rl 'CPD-OFF\|NOPMD' "$SRC_DIR" 2>/dev/null | sort || true)
    log "Done. Save output to: $MODULE_DIR/src/test/resources/lint-suppression-allowlist.txt"
    exit 0
fi
